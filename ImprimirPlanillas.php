...
$sql1 = "SELECT * FROM torneo where status_torneo = 1 order by idtorneo DESC LIMIT 1";
		...
		$NOMBRETORNEO = utf8_decode($rows1['nombre_torneo']);	
  ...
  // var $y=15;
   //Cabecera de página
	...
	function NOMBREEQUIPO()
	{
		$this->ln(10);
		$this->SetFont('Arial','B',20);
		$this->Cell(300,5,'Nombre del Equipo :   '.$GLOBALS['d'].'          Cancha _______',0,0,'C');
	}
...
$contador=0;$a1=0;$a2=0;$a3=0;
 		   	$sql1="SELECT * FROM equipo where idequipo =1";
	    	$result1=mysqli_query($mysqli,$sql1); 
	    	while($mostrar1=mysqli_fetch_array($result1))
	    	{
	    		$nomequipo = utf8_decode($mostrar1['nombre_equipo']);
	    		$GLOBALS['d'] = utf8_decode($nomequipo);
	    		$pdf->AddPage();
				$pdf->ln(10);
				$pdf->utf8_decode(NOMBREEQUIPO());
				$pdf->ln(15); 
				$pdf->LINEA(); 
				$pdf->ln(5); 
				$pdf->LINEAJUGADOR(); 
...
$sql="SELECT * FROM equipojugadores ORDER BY idequipo";
    ...
    {   	         
		...
		/*$pdf->ln(2);
		$pdf->Cell(70,10,'valor de a1 '.$a1.' valor de contador '.$contador,1,0,'L');*/
		...
		if ($contador > 3) 
		{
			...
			$pdf->Cell(70,10,utf8_decode($nombrejugador),1,0,'L');
		...
		{  	
			...
			if ($contador==3) 
			{
				...
				$pdf->Cell(30,10,'TOTAL PUNTOS',0,0,'C');
				$pdf->Cell(20,10,'',1,0,'C');
			}
		}
	}
