...
$sql="select sum(score) as TOTAL,idequipo from `lineas` where score > 0 and idtorneo = $idtorneo and tipo_torneo = 2 group by idequipo order by sum(score) DESC";
...
<caption class="text-center">	
	...
	<strong><font color="black">Resultado Parejas</font></strong><br>
   <tr>
    <th>&nbsp;</th>
...
<?php
/*
$sqlW="SELECT count(*) as CUANTOSHAY FROM `equipo` WHERE tipo_torneo = 2";
...
/*
			...
			idequipo = $EQUIPO and tipo_torneo = 2";
...
//		echo "<br>  nombre $nomb[$ix] <br>";
		...
		echo "<tr>";
			...
			ced_jugador=$cedu[$ix] and idequipo > 0 and tipo_torneo = 2 and idtorneo = $idtorneo order by fecha_linea, idlineas";
		$result1=mysqli_query($mysqli,$sql1);
		while($mostrar1=mysqli_fetch_array($result1))
		{ 
...
if ($jugadas ==20) 
	...
	$sqlp="SELECT sum(score) as sumapar FROM `lineas` WHERE `idequipo` = $idequipo and tipo_torneo = 2 and idtorneo = $idtorneo group by fecha_linea, idlineas";
