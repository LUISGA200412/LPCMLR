...
{
	echo  "<script>
	alert('el jugador ==> $saljugador, ya tiene equipo asignado' );
	location.href ='CargarParejas.php';  </script>";   
}
else
{
		...
		$fecha=date("Y-m-d"); 	
		$i=0;
		$sql = "insert into equipo( nombre_equipo,tipo_torneo ) 
				values ( '$equipo','2'	)";
		...
		{ 
			...
			else
			{
				...
				alert('Error: CARGANDO JUGADOR $sql2 <br> $mysqli->error');
						location.href ='CargarParejas.php';
					</script>";
			}
		}	 
 	}
