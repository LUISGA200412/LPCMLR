...
$sql1 = "SELECT * FROM torneo where status_torneo = 1 and tipo_torneo = 4 order by idtorneo DESC LIMIT 1";
...
if (empty($idtorneo)) {
	echo "<script> alert('Aun No se ha cargado Ningun Torneo');
		...
		$sql="select sum(score) as TOTAL,idequipo from `lineas` where score > 0 and tipo_torneo = 4 and idtorneo = $idtorneo group by idequipo order by sum(score) DESC";
...
$sqlW="SELECT count(*) as CUANTOSHAY FROM `equipo` WHERE tipo_torneo = 4";
...
//{
	...
	$sumapar = array();
//		echo "<br> $i <br>";
		$sql="select sum(score) as TOTAL,idequipo from `lineas` where score > 0 and tipo_torneo = 4 group by idequipo order by sum(score) DESC";
...
//		echo "<br> $EQUIPO <br>";	
			...
			idequipo = $EQUIPO and tipo_torneo = 4";
...
//		echo "<br>  nombre $nomb[$ix] <br>";
		...
		echo "<tr>";
			...
			ced_jugador=$cedu[$ix] and idequipo > 0 and tipo_torneo = 4 order by fecha_linea, idlineas";
		$result1=mysqli_query($mysqli,$sql1);
		while($mostrar1=mysqli_fetch_array($result1))
		{ 
...
if ($jugadas ==20) 
	...
	$sqlp="SELECT sum(score) as sumapar FROM `lineas` WHERE `idequipo` = $idequipo and tipo_torneo = 4  group by fecha_linea, idlineas";
