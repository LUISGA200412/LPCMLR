...
sum(score) as SUMATOTAL, `ced_jugador`, idequipo from `lineas` where  sexo = 1 and 
score > 0 and tipo_torneo = $tipotorneo and idtorneo = $idtorneo 
...
$sql1 = "SELECT * FROM torneo where status_torneo = 1 and tipo_torneo = $tipotorneo order by idtorneo DESC LIMIT 1";
		$resultado1 = $mysqli->query($sql1);
		while($rows1 = $resultado1->fetch_assoc())
		{
		$idtorneo = $rows1['idtorneo'];	
...
{ 
		...
		`ced_jugador`=$cedula and sexo = 1 and tipo_torneo = $tipotorneo and 
