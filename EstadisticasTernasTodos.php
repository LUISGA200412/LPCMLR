...
$sql="select sum(score) as TOTAL,idequipo from `lineas` where score > 0 and idtorneo = $idtorneo and tipo_torneo = 3 group by idequipo order by sum(score) DESC";
...
<caption class="text-center">
	...
	<strong><font color="black">Resultado Ternas</font></strong><br>
...
*/
			...
			$sqlq="select * from `lineas` where  idequipo = $EQUIPO and idtorneo = $idtorneo and tipo_torneo = 3 group by ced_jugador";
...
//	score > 0 and
		 ...
		 ced_jugador=$cedu[$ix] and idequipo > 0 and idtorneo = $idtorneo and tipo_torneo = 3 order by fecha_linea, idlineas";
		$result1=mysqli_query($mysqli,$sql1);
		while($mostrar1=mysqli_fetch_array($result1))
		{ 
...
if ($jugadas ==20) 
	...
	$sqlp="SELECT sum(score) as sumapar FROM `lineas` WHERE score > 0 and `idequipo` = $idequipo and idtorneo = $idtorneo and tipo_torneo = 3 group by fecha_linea, idlineas";
