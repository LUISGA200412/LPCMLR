...
$sql="select sum(score) as TOTAL,idequipo from `lineas` where score > 0 and idtorneo = $idtorneo and tipo_torneo = 4 group by idequipo order by sum(score) DESC";
...
<caption class="text-center">
	...
	<strong><font color="black">Resultado Cuartas</font></strong><br>
...
*/
			...
			$sqlq="select * from `lineas` where  idequipo = $EQUIPO and idtorneo = $idtorneo and tipo_torneo = 4 group by ced_jugador";
...
//print_r($par);
		...
		array_push($parejas, "$promedioequipo", "$idequipo", "$nombrequipo", "$cedula1", "$nombrejugador1", "$cedula2", "$nombrejugador2" , "$cedula3", "$nombrejugador3" , "$cedula4", "$nombrejugador4");
...
//	score > 0 and
		 ...
		 ced_jugador=$cedu[$ix] and idequipo > 0 and idtorneo = $idtorneo and tipo_torneo = 4 order by fecha_linea, idlineas";
		$result1=mysqli_query($mysqli,$sql1);
		while($mostrar1=mysqli_fetch_array($result1))
		{ 
...
if ($jugadas ==20) 
	...
	$sqlp="SELECT sum(score) as sumapar FROM `lineas` WHERE score > 0 and `idequipo` = $idequipo and idtorneo = $idtorneo and tipo_torneo = 4 group by fecha_linea, idlineas";
