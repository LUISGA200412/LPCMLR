...
sum(score) as SUMATOTAL, `ced_jugador`, idequipo from `lineas` where  sexo = 2 
and score > 0 and tipo_torneo = $tipotorneo and idtorneo = $idtorneo  
group by `ced_jugador` order by avg(score) DESC";
...
$sql1 = "SELECT * FROM torneo where status_torneo = 1 and tipo_torneo = $tipotorneo 
 order by idtorneo DESC LIMIT 1";
		$resultado1 = $mysqli->query($sql1);
		while($rows1 = $resultado1->fetch_assoc())
		{
		$idtorneo = $rows1['idtorneo'];	
...
<caption class="text-center">
	<strong><font color="black"><?php echo $nomtorneo; ?></font></strong><br>
Individual Femenino
...
$j=0;
	...
	{ 
		...
		`ced_jugador`=$cedula and sexo = 2 and tipo_torneo = $tipotorneo and idtorneo = $idtorneo 
		order by fecha_linea, idlineas";
		echo "<tr>";	
		...
		//	$suma = $mostrar1['score'] + $suma;
			...
			if ($jugadas==1) 
			{
				...
				if ($i ==0) {
					...
					$nombrejugador</td>
					<td style='vertical-align:middle;'>$nombrecorto
					</td>";
				...
				if ($i ==1) {
					...
					$nombrejugador</td>
					<td style='vertical-align:middle;'>$nombrecorto</td>";
					echo '<td  align="center"> ' .
					     '<img width="50" height="50" src = "data:image/png;base64,' . base64_encode($foto) . '" />'
					      . '</td>';
