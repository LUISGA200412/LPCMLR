...
$sql1 = "SELECT * FROM torneo where status_torneo = 1 and tipo_torneo = 2 order by idtorneo DESC LIMIT 1";
...
//{
	...
	$sumapar = array();
//		echo "<br> $i <br>";
		$sql="select sum(score) as TOTAL,idequipo from `lineas` where score > 0 and tipo_torneo = 2 group by idequipo order by sum(score) DESC";
...
//		echo "<br>  nombre $nomb[$ix] <br>";
		...
		echo "<tr>";
			...
			ced_jugador=$cedu[$ix] and idequipo > 0 and tipo_torneo = 2 order by fecha_linea, idlineas";
		$result1=mysqli_query($mysqli,$sql1);
		while($mostrar1=mysqli_fetch_array($result1))
		{ 
...
if ($jugadas ==20) 
	...
	$sqlp="SELECT sum(score) as sumapar FROM `lineas` WHERE `idequipo` = $idequipo and tipo_torneo = 2 group by fecha_linea, idlineas";
