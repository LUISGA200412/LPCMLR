...
Parejas Masculinos Super Senior
 

   <tr>
    <th>&nbsp;</th>
    <th class="hidden-xs text-center">Jugador</th>
    <th class="hidden-xs text-center">Entidad</th>
...
$sqlW="SELECT count(*) as CUANTOSHAY FROM `equipo` where sexo = 1";
...
//{
	...
	$sumapar = array();
//		echo "<br> $i <br>";
		$sql="select sum(score) as TOTAL,idequipo from `lineas` where sexo = 1 and 
		     idsubcategoria= 1 and idequipo > 0 group by idequipo order by sum(score) DESC";
	...
	$sumapar = array();
			...
			$TOTAL 		= $mostrar['TOTAL']; 
			$EQUIPO 	= utf8_encode($mostrar['idequipo']);
//		echo "<br> $EQUIPO <br>";	

			$sql1="select * from equipo where 
			idequipo = $EQUIPO and sexo = 1";
			$result1=mysqli_query($mysqli,$sql1);
			while($mostrar1=mysqli_fetch_array($result1))
			{ 
				$NOMBREEQUIPO	= utf8_encode($mostrar1['nombre_equipo']); 
//				echo "<br> $NOMBREEQUIPO <br>";
			}

			$sqlq="select * from equipojugadores  where 
			idequipo = $EQUIPO and sexo = 1 and idsubcategoria = 1 order by ced_jugador";
...
//echo "<br> =============== ";
		...
		array_push($parejas, "$promedioequipo", "$idequipo", "$nombrequipo", "$cedula1", "$nombrejugador1", "$cedula2", "$nombrejugador2");
...
//		echo "<br>  nombre $nomb[$ix] <br>";
		...
		echo "<tr>";
			...
			ced_jugador=$cedu[$ix] and sexo = 1 and idsubcategoria = 1 and idequipo > 0 order by idlineas";
		$result1=mysqli_query($mysqli,$sql1);
		while($mostrar1=mysqli_fetch_array($result1))
		{ 
...
if ($jugadas ==12) 
	...
	$sqlp="SELECT sum(score) as sumapar FROM `lineas` WHERE `idequipo` = $idequipo and sexo = 1 and idsubcategoria = 1 group by idlineas";
...
echo "<td align='center'>$totalequipo</td>";
echo "<td align='center'>";
